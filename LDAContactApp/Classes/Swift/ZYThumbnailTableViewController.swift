...
import UIKit
public typealias ConfigureTableViewPreviewCellBlock = (_ cell: UITableViewCell, _ indexPath: IndexPath) -> UITableViewCell?
...
public typealias willDisplayCellBlock = (_ indexPath: IndexPath) -> Void?
...
open class ZYThumbnailTableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate ,UIGestureRecognizerDelegate{
    
    
    //MARK: DEFINE
    fileprivate static let TABLEVIEW_BACKGROUND_COLOR_DEFAULT = UIColor.white
     static let CELL_HEIGHT_DEFAULT = CGFloat(250.0)
    ...
    func refreshView()
    {
        ...
        self.mainTableView.layoutSubviews()
    ...
    open var ConfigureTableViewPreviewCellBlock: ConfigureTableViewPreviewCellBlock!
    
    /*
     = {
     return {
     assertionFailure("ERROR:  -  You must configure the ConfigureTableViewPreviewCellBlock")
     return nil;
     }
     }()
     */
    ...
    override open func viewDidLoad() {
        ...
        print(self.mainTableView.frame)
    ...
    self.mainTableView.frame.size = CGSize(width: self.view.frame.size.width, height: self.view.frame.size.height)
    ...
    var contentConstraintsObject : LDACAAnchorValue?
    

    
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        
        if self.currentPreViewCell != nil && self.currentPreviewCellTapGesture != nil
        {
        if gestureRecognizer == currentPreviewCellTapGesture
        {
            print(touch.view)
            print(currentPreViewCell)

            if (touch.view?.isDescendant(of: currentPreViewCell))!
            {
                return false
            }
        if currentPreViewCell.subviews.contains(touch.view!) ||  touch.view! ==  currentPreViewCell
            {
                return false
            }
        }
    ...
    var currentPreviewCellTapGesture : UITapGestureRecognizer!
    var currentPreViewCell : UIView!
    func previewCell(_ cell: UITableViewCell, indexPath: IndexPath) {
        ...
        currentPreviewCellTapGesture = tapGesture
       tapGesture.delegate = self as! UIGestureRecognizerDelegate
        ...
        let previewCell = ConfigureTableViewPreviewCellBlock(cell, indexPath)
        ...
        currentPreViewCell  = contentView
        //constraint
        ...
        contentConstraintsObject =    contentView.anchor(thumbnailView.topAnchor, left: thumbnailView.leftAnchor, bottom: thumbnailView.bottomAnchor, right: thumbnailView.rightAnchor, topConstant: 0, leftConstant: 0, bottomConstant: 0, rightConstant: 0, widthConstant: -1, heightConstant: -1)
    ...
    func dismissPreview() {
        ...
        UIView.animate(withDuration: 0.301992, animations: { () -> Void in
            ...
            if let thumnailview = self.thumbnailView
            {
    ...
    fileprivate func layoutTopView(_ indexPath: IndexPath) {
        ...
        topView.anchor(thumbnailView.topAnchor, left: thumbnailView.leftAnchor, bottom: contentView?.topAnchor, right: thumbnailView.rightAnchor, topConstant: 0, leftConstant: 0, bottomConstant: 0, rightConstant: 0, widthConstant: -1, heightConstant: -1)
        
        // contentView?.anchor(thumbnailView.topAnchor, left: thumbnailView.leftAnchor, bottom: thumbnailView.bottomAnchor, right: thumbnailView.rightAnchor, topConstant: 30, leftConstant: 0, bottomConstant: 0, rightConstant: 0, widthConstant: 0, heightConstant: 0)
        
        contentConstraintsObject?.anchorValueWithLDACAAnchorKey(.top).constant = 35
        
        self.thumbnailView.updateHeight(self.thumbnailView.bounds.height + topView.bounds.height + 20)
        self.handleOverFlowScreen(self.thumbnailView)
    ...
    open var willDisplayCellAction : willDisplayCellBlock!
    
    func willDisplayCellAction(action:@escaping willDisplayCellBlock)
    {
        self.willDisplayCellAction = action
    }

    public func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
    
        if willDisplayCellAction != nil
        {
            self.willDisplayCellAction(indexPath)
        }
