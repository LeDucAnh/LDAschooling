...
class ConversationsVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func customization()  {
        ...
        if let id = String(Int((FirebaseAuthAPI.shareInstance.currentLDACAUSer?.roleID)!)) as? String {
           /*
            FirebaseAuthAPI.shareInstance.info(forUserID: id, completion: {
                [weak weakSelf = self]
                (LDACAUser) in
               
            
            cell.profilePic.sd_setImage(with: URL(string: self.items[indexPath.row].user.profileImageURL))
 ...
 */
    ...
    func pushToUserMesssages(notification: NSNotification) {
        if let user = notification.userInfo?["user"] as? LDACAUser {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toUserChatVC" {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        default:
         ...
         //   cell.profilePic.image = self.items[indexPath.row].user.profilePic
            ...
            cell.nameLabel.text = self.items[indexPath.row].user.firstName + " " + self.items[indexPath.row].user.lastName
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.items.count > 0 {
            ...
            self.performSegue(withIdentifier: "toUserChatVC", sender: self)
        }
    }
