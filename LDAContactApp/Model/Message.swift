...
class Message {
    ...
    class func downloadAllMessages(forUserID: String, completion: @escaping (Message) -> Swift.Void) {
        ...
        if let currentUserID = String(Int((FirebaseAuthAPI.shareInstance.currentLDACAUSer?.roleID!)!)) as? String  {
            ...
            FIRDatabase.database().reference().child("LDACAUsers").child(currentUserID.description).child("conversations").child(forUserID.description).observe(.value, with: { (snapshot) in
                if snapshot.exists() {
    ...
    class func markMessagesRead(forUserID: String)  {
        if let currentUserID = String(Int((FirebaseAuthAPI.shareInstance.currentLDACAUSer?.roleID!)!)) as? String {
            FIRDatabase.database().reference().child("LDACAUsers").child(currentUserID.description).child("conversations").child(forUserID.description).observeSingleEvent(of: .value, with: { (snapshot) in
                if snapshot.exists() {
                    let data = snapshot.value as! [String: String]
                    let location = data["location"]!
    ...
    class func uploadMessage(withValues: [String: Any], toID: String, completion: @escaping (Bool) -> Swift.Void) {
        if let currentUserID = String(Int((FirebaseAuthAPI.shareInstance.currentLDACAUSer?.roleID)!)) as? String {
            FIRDatabase.database().reference().child("LDACAUsers").child(currentUserID).child("conversations").child(toID).observeSingleEvent(of: .value, with: { (snapshot) in
                if snapshot.exists() {
                    let data = snapshot.value as! [String: String]
                    let location = data["location"]!
                    ...
                    FIRDatabase.database().reference().child("conversations").childByAutoId().childByAutoId().setValue(withValues, withCompletionBlock: { (error, reference) in
                        ...
                        FIRDatabase.database().reference().child("LDACAUsers").child(currentUserID).child("conversations").child(toID).updateChildValues(data)
                        FIRDatabase.database().reference().child("LDACAUsers").child(toID).child("conversations").child(currentUserID).updateChildValues(data)
                        completion(true)
                    })
                }
            })
