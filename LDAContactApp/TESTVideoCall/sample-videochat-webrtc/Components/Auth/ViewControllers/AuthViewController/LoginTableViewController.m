...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = NSLocalizedString(@"Enter to call", nil);
    
    [self defaultConfiguration];
    //Update interface and start login if user exist
    self.userNameTextField.text = self.chatUserName;
    

   // self.userNameTextField.userInteractionEnabled = false;
    self.chatRoomDescritptionLabel.text = @"Enter Private key that you want";
            self.chatRoomNameTextField.text = _chatRoom;
  //  [self login];
...
- (void)defaultConfiguration {
    ...
    void (^updateLoginInfo)(QBNetworkStatus status) = ^(QBNetworkStatus status) {
        ...
        NSLocalizedString(@"Please enter your username and call room name. You can join existent chat room.", nil);
...
- (void)login {
    ...
    if (Core.currentUser && [Core.currentUser.fullName isEqualToString:_userNameTextField.text])
  {
        ...
        NSLog(@"%@", _chatRoomNameTextField.text);
